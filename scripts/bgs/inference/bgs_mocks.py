...
def compress_mock_x(
    ...
    hod_params = params.get('hod_params', {}).get(f'Mr{Mr_cut}', {})
    ...
    if sigma_to_log10 and 'sigma' in parameters:
...
def compress_mock_tpcf(
    ...
    add_covariance : bool, optional
        Whether to compute and include the covariance array. Defaults to True.
    ...
    if mock_type == 'SecondGen':
        ...
        data_dir = Path(UCHUU_DIR)
        mock_name = '' # No mock subfolders for Uchuu
    else:
        raise ValueError(f'Unknown mock type: {mock_type}')

    fns = [data_dir / f'z{z:.3f}' / mock_name / 'measurements' / f'Mr{Mr_cut}' / f'tpcf_los_{_l}.npy' for _l in los]
    ...
    if add_covariance:
        ...
        for phase_cov in phase_idx:
            ...
            fns_cov = [data_dir / f'z{z:.3f}' / mock_name_cov / 'measurements' / f'Mr{Mr_cut}' / f'tpcf_los_{_l}.npy' for _l in los]
...
def compress_mock_ds(
    ...
    fns = [data_dir / f'z{z:.3f}' / mock_name / 'measurements' / f'Mr{Mr_cut}' / f'{measurement_root}_los_{_l}.npy' for _l in los]
    y_quantiles = []
    for q in quantiles:
    ...
    if add_covariance:
        ...
        for phase_cov in phase_idx:
            ...
            fns_cov = [data_dir / f'z{z:.3f}' / mock_name_cov / 'measurements' / f'Mr{Mr_cut}' / f'{measurement_root}_los_{_l}.npy' for _l in los]
            y_quantiles = []
            for q in quantiles:
...
def compress_mock(stat_name: str, **kwargs):
    ...
    **kwargs
    ...
    elif stat_name.startswith('ds_xi'):
        if 'xiqq' in stat_name:
            measurement_root = 'quantile_correlation'
        elif 'xiqg' in stat_name:
            measurement_root = 'quantile_data_correlation'
        if '_r3' in stat_name:
            kwargs['rebin'] = 3
        dataset = compress_mock_ds(measurement_root=measurement_root, **kwargs)
    # TODO: Add Fourier-space methods
    else:
        raise ValueError(f"Unknown statistic name: {stat_name}")
    return dataset

def check_datasets_compatibility(*observables, x_names: bool=False, covariance: bool=False) -> None:
    """
    ...
    *observables : Observable
    ...
    x_names : bool, optional
        Whether to check the compatibility of x_names. Defaults to False.
    covariance : bool, optional
        Whether to check the compatibility of covariance matrices. Defaults to False.
    ...
    for obs in observables[1:]:
        if x_names and ref.x_names != obs.x_names:
...
def get_mock_data(observable, return_obs: bool = False, check_args: dict = {}, **kwargs) -> tuple:
    """
    ...
    **kwargs
    ...
    add_covariance = kwargs.get('add_covariance', True)
    ...
    for stat_name in stat_names:
        ...
        check_datasets_compatibility(target_obs, tmp_obs, **check_args)
    ...
    check_datasets_compatibility(observable, mock_observable, **check_args)
