...
#SBATCH --job-name bgs-21.35_measure_base_outliers
...
LOGFILE="/pscratch/sd/s/sbouchar/acm/bgs-21.35/measurements/logs/outliers/log_${SIMTYPE}_${METHOD}.log"
OUTLIERS="jobs/bgs-21.35/outliers/${METHOD}-simtype_${SIMTYPE}-all_measurements.npy"

python measure_box.py --config jobs/bgs-21.35/config.yaml --gpu --log_file "${LOGFILE}" --overwrite --parameters_override "${OUTLIERS}" #--measurements power_spectrum density_split_power
