...
if __name__ == "__main__":
    ...
    # Load outliers info NOTE: This is the main change compared to measure_box.py
    outlier_dir = Path('/global/homes/s/sbouchar/acm/scripts/bgs/measurements/outliers')
    outliers = {}
    if 'tpcf' in measurements:
        outliers['tpcf'] = np.load(outlier_dir / f'tpcf_outliers_simtype-{sim_type}_ells-02_sigma-10.0.npy')
    if 'density_split' in measurements:
        outliers1 = np.load(outlier_dir / f'ds_xigg_outliers_simtype-{sim_type}_ells-02_sigma-10.0.npy')
        outliers2 = np.load(outlier_dir / f'ds_xiqg_outliers_simtype-{sim_type}_ells-02_sigma-10.0.npy')
        outliers['density_split'] = np.unique(np.concatenate((outliers1, outliers2)))
    outliers['all'] = np.unique(np.concatenate(list(outliers.values()))) if len(outliers) > 0 else np.array([])
    
    logger.info(f'Found {len(outliers["all"])} outlier HODs across all measurements.')
    ...
    for cosmo_idx in cosmologies:
        ...
        # NOTE: Changed to only process outlier HODs
        hods = outliers['all']['hod'][outliers['all']['cosmo_idx'] == cosmo_idx]
        tpcf_hods = outliers['tpcf']['hod'][outliers['tpcf']['cosmo_idx'] == cosmo_idx] if 'tpcf' in outliers else np.array([])
        density_split_hods = outliers['density_split']['hod'][outliers['density_split']['cosmo_idx'] == cosmo_idx] if 'density_split' in outliers else np.array([])
        logger.info(f'Processing {len(hods)} outlier HODs for cosmology {cosmo_idx}: {hods}')
            ...
            for seed in seeds:
                ...
                for hod_idx in hods:
                    ...
                    for los in ['x', 'y', 'z']:
                        ...
                        if 'tpcf' in measurements and hod_idx in tpcf_hods:
                        ...
                        if 'density_split' in measurements and hod_idx in density_split_hods:
