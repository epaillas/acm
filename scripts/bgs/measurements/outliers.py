...
def get_outliers(group: ObjectGroup, *names: str, **kwargs) -> np.ndarray:
    """Find outliers in the compressed data through sigma clipping."""
    ...
    mask = clipped_data.mask.any(axis=1)  # ty:ignore[unresolved-attribute]
...
def check_missing_files(
    ...
    **expected: list,
    ...
    expected_iterable = itertools.product(*expected.values())
    missing_idx = [list(idx) for idx in expected_iterable if idx not in values]
    logger.info(
        f"Found {len(missing_idx)} missing files out of {n_expected if n_expected is not None else nfiles}."
    )
...
if __name__ == "__main__":
    ...
    if args.save_dir:
        ...
        for arr, idx_type in zip(
            [corrupted_idx, outlier_idx],
            ["corrupted", "outlier"],
            strict=True,
        ):
            fn = save_dir / f"{stat_name}_{idx_type}_idx.csv"
            if len(arr) == 0:
                continue
            logger.info(f"Saving {len(arr)} {idx_type} indices to {fn}")
            np.savetxt(fn, arr, delimiter=",", fmt="%d", header=header)
