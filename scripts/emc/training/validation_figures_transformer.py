...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Generate validation figures for EMC transformer emulators.",
    )
    parser.add_argument(
        "--module",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional transformer model root or statistic-specific directory. "
            "When omitted, uses the default transformer best-model namespace."
        ),
    )
    return parser
...
def resolve_checkpoint_path(statistic, model_dir):
    ...
    raise FileNotFoundError(
        f"Could not find a transformer checkpoint for '{statistic}'. Tried:\n"
        f"{attempted_paths}"
    )
