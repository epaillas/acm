...
def get_spectrum_fns(cosmo_idx=0, phase_idx=0, n_hod=1):
...
def compare_spectrum_vs_recon_spectrum():
    pk_fns = np.concatenate([get_spectrum_fns(cosmo_idx=i, n_hod=3) for i in range(1)])
    recon_pk_fns = np.concatenate([get_recon_spectrum_fns(cosmo_idx=i, n_hod=3) for i in range(1)])
    k, _ = read_spectrum(pk_fns[0])
    poles = [read_spectrum(fn)[1] for fn in pk_fns]
...
def compare_tpcf_vs_recon_tpcf():
    tpcf_fns = np.concatenate([get_tpcf_fns(cosmo_idx=i, n_hod=3) for i in range(1)])
    recon_tpcf_fns = np.concatenate([get_recon_tpcf_fns(cosmo_idx=i, n_hod=3) for i in range(1)])
    s, _ = read_tpcf(tpcf_fns[0])
    poles = [read_tpcf(fn)[1] for fn in tpcf_fns]
