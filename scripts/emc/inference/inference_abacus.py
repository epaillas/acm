...
def save_and_plot(sampler, observable):
    ...
    sampler.save_chain(
        save_fn=save_dir / f"chain_{statistics}.npy",
    ...
    chain.plot_triangle(
        save_fn=save_dir / f"chain_{statistics}_triangle.pdf",
        thin=128,
    ...
    observable.plot_observable(
        model_params=chain.bestfit, save_fn=save_dir / f"chain_{statistics}_bestfit.pdf"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--statistics", nargs="+", help="List of statistics to use, e.g. spectrum"
    )
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/global/cfs/cdirs/desicollab/users/epaillas/acm/emc/fits/abacus/apr20",
    )
