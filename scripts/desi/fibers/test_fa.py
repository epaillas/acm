...
if __name__ == '__main__':
    ...
    if mpicomm.rank == 0:
        ...
        cutsky.sample_hod(hod, nthreads=1, region='NGC', release='Y1')
        ...
        cutsky.catalog = None
    mpicomm.Barrier()
    cutsky.run_FA_for_single_tracer(release='Y1', program='dark', npasses=7, use_sky_targets=False, 
                                    seed=None, preload_sky_targets=False, plot_output=True, mpicomm=mpicomm)

    # save the cutsky catalog to disk (optional, for debugging)
    pscratch = os.environ["PSCRATCH"]

    cutsky.catalog.save(f'{pscratch}/cutsky_FA.fits')
