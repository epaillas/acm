...
class MinkowskiFunctionals(BasePyreconMeshEstimator):
    ...
    def set_density_contrast(self, global_mean=None,smoothing_radius=None, check=False, ran_min=0.01, save_wisdom=False, plan='estimate'):
        """
        ...
        t0 = time.time()
        if smoothing_radius:
            self.data_mesh.smooth_gaussian(smoothing_radius, engine='fftw', save_wisdom=save_wisdom,plan=plan)
        if self.has_randoms:
            if check:
            ...
            if smoothing_radius:
                self.randoms_mesh.smooth_gaussian(smoothing_radius, engine='fftw', save_wisdom=save_wisdom,plan=plan)
            ...
            self.delta_mesh = self.data_mesh / global_mean - 1.
        else:
    ...
    def get_rho(self, rho_thres, smoothing_radius=None, save_wisdom=False, plan='estimate'):
        """
         get rho.
        ...
        if smoothing_radius:
            self.data_mesh.smooth_gaussian(smoothing_radius, engine='fftw', save_wisdom=save_wisdom,plan=plan)
            self.mask_mesh.smooth_gaussian(smoothing_radius, engine='fftw', save_wisdom=save_wisdom,plan=plan)
    ...
    def run(self,thres_mask=-2,thresholds = np.linspace(-1,5,num=201,dtype=np.float32),nthreads=1):
        """
        ...
        if nthreads==1:
            mf = Mk.MFs(self.delta_mesh,self.data_mesh.cellsize[0],self.thres_mask,self.thresholds)
        else:
            mf = Mk.MFs_parallel(self.delta_mesh,self.data_mesh.cellsize[0],self.thres_mask,self.thresholds,nthreads)
        self.MFs = mf.MFs3D
    ...
    def measure_v123(self,thres_mask=-2, thres_bins=200):
        ...
        t0 = time.time()
        self.thres_mask = thres_mask
        self.thresholds = np.quantile(self.rho_used,np.linspace(0,1,num=thres_bins+1))
        mf = Mk.MFs(self.rho_mesh,self.data_mesh.cellsize[0],self.thres_mask,np.float32(self.thresholds))
        self.MFs = mf.MFs3D
        self.logger.info(f"Minkowski functionals elapsed in {time.time() - t0:.2f} seconds.")
        return self.MFs
    ...
    def plot_V123(self,x=[],label="MFs",mf_cons=[10**2,10**3,10**4]):
        ...
        ylabels = [r"$V_{1}[10^{- "+str(int(np.log10(mf_cons[0])))+"}hMpc^{-1}]$",
