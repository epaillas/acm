...
class GalaxyMultiplets:
    ...
    def find_pairs(self, positions, boxsize):
        ...
        """
        self.logger.info(f'Finding pairs with r_max={self.r_max}, r_perp_max={self.r_perp_max}')
        ...
        sep_3d = np.sqrt(np.sum((positions[all_pairs[:, 0]] - positions[all_pairs[:, 1]])**2, axis=1))
        ...
        if len(non_boundary_pairs) > 0:
            proj_sep_nb = np.sqrt(np.sum(
                (positions[non_boundary_pairs[:, 0]][:, perp_axes] - 
                 positions[non_boundary_pairs[:, 1]][:, perp_axes])**2, axis=1
            ))
        ...
        pairs = np.vstack([non_boundary_pairs, boundary_pairs]) if len(non_boundary_pairs) > 0 else boundary_pairs
        
        self.logger.info(f'Found {len(pairs)} pairs satisfying all criteria')
        self.logger.info(f'  Non-boundary: {len(non_boundary_pairs)}, Boundary: {len(boundary_pairs)}')
    ...
    def identify_multiplets(self, positions, boxsize):
        ...
        self.logger.info(f'Found {len(self.singlet_ids)} singlets ({len(self.singlet_ids)/n_galaxies*100:.2f}%)')
        self.logger.info(f'{len(all_pair_ids)/n_galaxies*100:.2f}% of galaxies are in multiplets')
    ...
    def compute_cross_correlation(self, multiplet_coords, all_positions, boxsize, edges):
        """
        ...
        DD_counts = TwoPointCounter(
            'rppi', edges,
            positions1=[multiplet_coords[:, 0], multiplet_coords[:, 1], multiplet_coords[:, 2]],
            positions2=[all_positions[:, 0], all_positions[:, 1], all_positions[:, 2]],
            boxsize=boxsize, los=self.los,
            position_type='xyz', engine='corrfunc', nthreads=self.nthreads
        )
        
        RR_counts = AnalyticTwoPointCounter(
            'rppi', edges,
            size1=len(multiplet_coords), size2=len(all_positions),
            boxsize=boxsize, los=self.los
        )
    ...
    def compute_all_cross_correlations(self, positions, boxsize, edges, pimax=None):
        ...
        if len(self.singlet_coords) > 0:
            self.logger.info(f'Computing singlet cross-correlation ({len(self.singlet_coords)} objects)')
        ...
        if len(self.pair_coords) > 0:
            self.logger.info(f'Computing pair cross-correlation ({len(self.pair_coords)} objects)')
        ...
        if len(self.triplet_coords) > 0:
            self.logger.info(f'Computing triplet cross-correlation ({len(self.triplet_coords)} objects)')
        ...
        if len(self.quadruplet_coords) > 0:
            self.logger.info(f'Computing quadruplet cross-correlation ({len(self.quadruplet_coords)} objects)')
