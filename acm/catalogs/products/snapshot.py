...
def _apply_downsample(
    ...
    if f_gal is not None:
        ...
        if boxsize is None:
            raise ValueError("boxsize function must be provided when downsampling by nbar.")
        volume = np.prod(boxsize()) # Callable to get current boxsize, which may include AP scaling
        n_target = int(nbar * volume)

    if n_target >= n_current:
        logger.warning(f"Target n_gal={n_target} >= current n_gal={n_current} for tracer '{tracer}', skipping downsample.")
        return data
...
class SnapshotCatalog(BaseGalaxyCatalog):
    ...
    def boxsize(self) -> np.ndarray[tuple[int, int, int], float]:
        ...
        if "ap" in self._transforms:
            ...
            los = ap.kwargs["los"]
            factors = np.array([self.q_par if ax == los else self.q_perp for ax in self.pos_columns])
    ...
    def boxsize(self, value: float | list[float] | np.ndarray) -> None:
        ...
        if value.ndim == 0:  # scalar
            ...
            raise ValueError(f"boxsize must be a scalar or a 3-element array, got shape {value.shape}.")
