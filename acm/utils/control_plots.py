...
class ObservablePlots():
    ...
    def load_observable(self, observable: Observable, label:str = None):
        ...
        if label is not None:
            ...
            self.index_observable_dict[observable.stat_name] = len(self.observables)-1
        return observable
    
    def plot_y(self, label: str, **kwargs):
        """
        Plots the y-values for a given observable.
        ...
        **kwargs : dict, optional
            ...
            If a 'factor' keyword argument is provided, the y-values will be scaled by this factor (float or array of the size of the length of the data vector).
        ...
        Notes
        -----
        The function retrieves the y-values for the specified observable, scales them by the given factor,
        ...
        y *= f
        ax.plot(y.T, **kwargs) # Transpose to plot the different observables
        
        ax.set_xlabel('bin_value index')
        ax.set_ylabel('Observable')
        ax.set_title(f'Loaded y for observable {label}')
        fig.tight_layout()
        
        return fig, ax
    
    def plot_x(self, label: str, names: list[str] = None, return_histograms: bool = False, **kwargs):
        """
        Plots histograms of x-values for a given observable label.
        ...
        names : list of str, optional
            The names of the x-values to plot. If None, all x-values will be plotted.
            Defaults to None.
        ...
        **kwargs : dict
        ...
        histograms : dict, optional
            A dictionary containing the histograms for each x-value name. Only returned if `return_histograms` is True.
        ...
        for i, name in enumerate(names):
            index = x_names.index(name)
            h = ax[i].hist(x[:, index], **kwargs)
            histograms[name] = h
    ...
    def get_separators(self, observable: CombinedObservable):
        """
        ...
        if not isinstance(observable, CombinedObservable):
            return []
        
        # Separations
        separations = []
        for s in observable.observables:
            data_length = s.y.shape[-1] # length of the data vector is the last dimension of the y array (n_bins after filtering and flattening over statistic dimensions)
    ...
    def plot_model(self, label: str, truth: int|np.ndarray, params: list = None):
        ...
        truth : int | np.ndarray
            The truth to plot. If an integer is provided, the function will use the y-value at this index.
        ...
        params : list, optional
            ...
            If None, the truth will be used (as the x-value at the given index).
            Defaults to None.
        ...
        sigma = np.sqrt(np.diag(covariance_matrix) + emulator_error**2) # total error is the combination of the emulator error and the data error
