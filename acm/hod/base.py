...
def map_params(params: dict | list[str], mapping: dict[str, list[str]] = ABACUS_MAP) -> dict | list[str]:
    """
...
class BaseGalaxycatalog:
    ...
    logger = logging.getLogger('acm.hod.BaseGalaxycatalog') # Set up logger for the class as a class attribute
    ...
    def __init__(
        ...
        self.cosmo_fid = fiducial_cosmology if fiducial_cosmology is not None else DESI() # Fiducial cosmology (for AP parameters)
    ...
    def __getitem__(self, key):
        if isinstance(key, str):
            ...
            raise KeyError('Invalid key type. Key must be a string representing the tracer name.')
    
    def get_tracer_catalog(self, tracer: str) -> dict:
        if tracer not in self.catalogs:
            raise ValueError(f'Tracer {tracer} not found in the galaxy catalog. Available tracers: {list(self.catalogs.keys())}.')
...
class BaseHOD:
    ...
    logger = logging.getLogger('acm.hod.BaseHOD') # Set up logger for the class as a class attribute
    ...
    def __init__(
        ...
        hod_setup_params: dict | None = None, # TODO: Find a better name, as we will use hod_params with another method element
        ...
        **kwargs
        ...
        if config_file is None:
            config_file = Path(config_dir) / 'box.yaml' # TODO: change this to be handled in child classes with different default config files
            self.logger.info(f"No config file provided. Using default config: {config_file}.")
        
        defaults = yaml.safe_load(open(config_file))
        
        for d, n in zip([sim_params, hod_setup_params], ['sim_params', 'hod_setup_params']):
            if d is None:
                d = {}
            d.update(defaults.pop(n, {})) # Remove elements from defaults after updating
        ...
        if load_balls:
            self._load_balls(snapshots) # Load AbacusHOD balls for the specified snapshots
    ...
    def _sim_params_override(
        ...
        if cosmo_idx is not None and phase_idx is not None and sim_type is not None:
            self.sim_params['sim_name'] = get_abacus_simname(sim_type, cosmo_idx, phase_idx)
    ...
    def _load_balls(self, snapshots: list[float]) -> None:
        ...
        for zsnap in snapshots:
            ...
            self.logger.info(f'Initializing AbacusHOD ball with parameters {sim_params}.')
    ...
    def get_ball(self, zsnap: float) -> AbacusHOD:
        ...
        if zsnap not in self.snapshots:
            raise ValueError(f'Snapshot {zsnap} not found in loaded snapshots: {self.snapshots}.')
