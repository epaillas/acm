...
class BoxHOD:
    ...
    def __init__(
        ...
        if sim_type not in ["base", "small", "png"]:
            raise ValueError(
                'Invalid sim_type. Must be either "base", "small", or "png".'
            )
    ...
    def abacus_simname(self) -> str:
        ...
        if self.sim_type == "png":
            return (
                f"Abacus_{self.sim_type}base_c{self.cosmo_idx:03}_ph{self.phase_idx:03}"
            )
        return (
    ...
    def run(
        ...
        tracer = self.tracer
        self.add_ap = (
            add_ap  # flag to indicate if AP distortions were applied to number density
        )
        if seed == 0:
            seed = None
        ...
        if tracer == "ELG" and self.sim_geometry == "cutsky":
            ...
            warnings.warn(
                "WARNING: ELG cutsky mocks are not currently supported. Unexpected behavior may occur."
            )
        ...
        if self.add_ap:
            self.n_gal *= self.q_par * self.q_perp**2
    ...
    def param_mapping(self, hod_params: dict | list[str]) -> dict | list[str]:
        ...
        if any(
            key in hod_params for key in custom_keys
        ):  # Same syntax for dict and list :)
            for abacus_key, custom_key in zip(abacus_keys, custom_keys):
                if (
                    custom_key in hod_params
                ):  # Just in case not all custom keys are used
                    ...
                    if type(hod_params) is dict:
                        ...
                        raise ValueError(
                            "Invalid type for hod_params. Must be either dict or list."
                        )
    ...
    def _add_rsd(
        ...
        tracer_dict[ax] = (
            pos_rsd - offset
        )  # Overwrite real-space positions with RSD positions
        return tracer_dict
