...
class ReconstructedGalaxyPowerSpectrumMultipoles(BaseObservableEMC):
    ...
    def __init__(self, stat_name: str = "recon_spectrum", **kwargs) -> None:
    ...
    def compress_covariance(
        ...
        data_fns = list(
            base_dir.glob("mesh2_recon_spectrum_poles_ph*.h5")
    ...
    def compress_data(
        ...
        if add_covariance:
            cov_y = cls.compress_covariance(
                paths=paths, stat_name=stat_name, rebin=rebin, ells=ells, overwrite_k=k
            )
    ...
    def plot_observable(self, model_params: dict, save_fn: str | None = None) -> tuple[plt.Figure, plt.Axes]:
        ...
        for i, ell in enumerate(ells):
            ...
            lax[i + 1].set_ylabel(
                rf"$\Delta P_{{{ell:d}}} / \sigma_{{ P_{{{ell:d}}} }}$", fontsize=15
            )
...
# Alias
recon_spectrum = ReconstructedGalaxyPowerSpectrumMultipoles
