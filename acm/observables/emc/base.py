...
class BaseObservableEMC(Observable):
    ...
    def __init__(
        ...
        if (
            ...
            and "model_dir" not in paths
            and "checkpoint_fn" not in kwargs
        ):
            try:
                kwargs["checkpoint_fn"] = lookup_registry_path(
                    "projects.yaml", "emc", "checkpoint_fn", stat_name
                )
            except KeyError:
    ...
    def get_hod_from_files(
        cls, paths: dict, cosmo_idx: int, phase: int = 0, seed: int = 0
    ) -> np.ndarray:
        """
    ...
    def compress_x(
        ...
        **kwargs,
        ...
        cosmo_file = paths[
            "cosmo_file"
        ]  #'/pscratch/sd/e/epaillas/emc/AbacusSummit.csv'
        hod_file = paths[
            "hod_file"
        ]  #'/pscratch/sd/n/ntbfin/emulator/hods/hod_params.npy'
        ...
        for cosmo_idx in cosmos:
            ...
            x_cosmo = np.array(
                [x_cosmo[param] for param in x_cosmo_names]
            )  # Enforce parameters ordering after mapping
