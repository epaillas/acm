...
class BaseObservableEMC(Observable):
    ...
    def plot_observable(self, model_params: dict, sample_idx: int = 0, save_fn: str = None):
        ...
        data = self.y[sample_idx] # Enforce 2D flattening and get a single sample
        bin_idx = np.arange(len(data))
        model = self.get_model_prediction(model_params)[0] # Enforce 2D flattening and get the first sample
