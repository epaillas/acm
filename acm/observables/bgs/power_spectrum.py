...
class PowerSpectrumMultipoles(BaseObservableBGS):
    ...
    def compress_data(
        ...
        **kwargs,
        ...
        if add_covariance:
            cov_y = cls.compress_covariance(paths=paths, stat_name=stat_name, rebin=rebin, kmin=kmin, kmax=kmax, ells=ells, overwrite_k=k, seed=seed, los=los, **kwargs)
    ...
    def plot_observable(self, model_params: dict, save_fn: str = None, ells: list = [0, 2], xscale: str = 'linear', **kwargs) -> tuple:
        """
        ...
        **kwargs : dict
        ...
        for i, ell in enumerate(ells):
            ...
            ax[0].errorbar(k, data*k**2, error*k**2, marker='o', ms=4, ls='', color=f'C{i}', elinewidth=1.0, capsize=None, label=fr'$\ell={ell}$')
