...
K_MAX = (
    np.pi * 397 / 2200
)  # Higher limit fixed by Nyquist frequency of the base boxsize (to be safe, we take the larger boxsize created, which has a lower Nyquist frequency) with 397^3 grid
    ...
    def compress_covariance(
        ...
        for phase in phases:
            ...
            for q in quantiles:
                ...
                data = (
                    data.select(k=slice(0, None, rebin))
                    .select(k=(kmin, kmax))
                    .get(quantiles=q)
                )
