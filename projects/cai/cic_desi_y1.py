...
def read_desi_data():
    data_dir = '/global/cfs/cdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v1.5/'
    data_fn = Path(data_dir) / f'LRG_{region}_clustering.dat.fits'
    return fitsio.read(data_fn)

def read_desi_randoms(irand=0):
    data_dir = '/global/cfs/cdirs/desi/survey/catalogs/Y1/LSS/iron/LSScats/v1.5/'
    data_fn = Path(data_dir) / f'LRG_{region}_{irand}_clustering.ran.fits'
    return fitsio.read(data_fn)
...
def get_cic_data(smoothing_radius=5.0):
    data = read_desi_data()
    data_positions, data_weights, data_mask = get_clustering_positions_weights(data, cattype='desi')
    ...
    for irand in range(18):
        randoms = read_desi_randoms(irand)
        randoms_positions, randoms_weights, randoms_mask = get_clustering_positions_weights(randoms, cattype='desi')
...
if __name__ == '__main__':
    ...
    for smoothing_radius in [5.0, 7.5, 10.0]:
        ...
        cic_data = get_cic_data(smoothing_radius=smoothing_radius)

        save_dir = '/pscratch/sd/e/epaillas/cic/'
        save_fn = Path(save_dir) / f'CIC_LRG_{region}_DESI_DR1_z{zmin}-{zmax}_sm{smoothing_radius}.npy'
