...
data_dir = f'/global/cfs/cdirs/desicollab/users/epaillas/acm/fits_emc/abacus/c000_hod030/base_w0'
data_fn = Path(data_dir) / f"pk+number_density_k0.00-0.50_chain.npy"
data = np.load(data_fn, allow_pickle=True).item()
samples = MCSamples(
            samples=data['samples'],
...
chains.append(samples)
maxl = data['samples'][data['log_likelihood'].argmax()]
legend_labels.append(r'$\textrm{Power spectrum}$')
...
g.plot_1d(chains, 'w0_fld', legend_labels=legend_labels) 
g.add_legend(legend_labels, legend_loc='upper right', facecolor='w', handlelength=1.0)
g.add_x_marker(data['markers']['w0_fld'], lw=2.0)
plt.savefig(f'fig/posterior_base_w0_c{cosmo_idx:03}_hod{hod_idx:03}.pdf', bbox_inches='tight')
plt.savefig(f'fig/posterior_base_w0_c{cosmo_idx:03}_hod{hod_idx:03}.png', bbox_inches='tight', dpi=300)