...
# chains.append(samples)
# maxl = data['samples'][data['log_likelihood'].argmax()]
# legend_labels.append(r'$B_\ell(k_1, k_2, k_3) 150HOD$')

data_dir = f'/global/cfs/cdirs/desicollab/users/epaillas/acm/fits_emc/abacus/c000_hod030/test_jan26/'
data_fn = Path(data_dir) / f"pk+bk_chain.npy"
data = np.load(data_fn, allow_pickle=True).item()
samples = MCSamples(
            samples=data['samples'],
            weights=data['weights'],
...
chains.append(samples)
maxl = data['samples'][data['log_likelihood'].argmax()]
legend_labels.append(r'$P_\ell(k) + B_\ell(k_1, k_2, k_3)$')
...
plt.savefig('pk_bk_contours.png', dpi=300, bbox_inches='tight')