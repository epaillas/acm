...
for ell in [0, 2]:
    ...
    ax[0, 3].errorbar(list(range(len(data_y)))[::3], data_y[::3], error[::3], markersize=3.0, elinewidth=1.0,
                    marker='o', ls='', color='dimgrey')
    # ax[0, 3].plot(list(range(len(model)))[::3], model[::3], color='r')
ax[0, 3].set_xlabel(r'$\textrm{bin index}$', fontsize=15)
ax[0, 3].set_ylabel(rf'$k_1k_2k_3B_{ell}(k_1, k_2, k_3)\,[h^{-3}{{\rm Mpc}}^3]$', fontsize=15)
...
for q in [0, 1, 3, 4]:
    ...
    ax[1, 0].errorbar(sep, sep**1*data_y, sep**1*error, markersize=3.0, elinewidth=1.0,
                    marker='o', ls='', color='dimgrey')
    # ax[1, 0].plot(sep, sep**1*model, color='r')
...
plt.tight_layout()
plt.savefig('diffsky_summaries_multipanel.pdf', bbox_inches='tight')