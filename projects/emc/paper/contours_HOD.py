...
# params = ['omega_cdm', 'sigma8_m', 'n_s']
params = ['logM_cut', 'logM_1', 'sigma', 'kappa', 'alpha', 's', 'alpha_s', 'alpha_c']
...
chains.append(samples)
maxl = data['samples'][data['log_likelihood'].argmax()]
legend_labels.append(r'$w(p) + P_\ell(k)$')

data_dir = f'/global/cfs/cdirs/desicollab/users/epaillas/acm/fits_emc/abacus/c000_hod030/base'
data_fn = Path(data_dir) / f"dsc_pk+minkowski+pk+wp+number_density_k0.00-0.50_chain.npy"
data = np.load(data_fn, allow_pickle=True).item()
samples = MCSamples(
            samples=data['samples'],
            weights=data['weights'],
...
chains.append(samples)
maxl = data['samples'][data['log_likelihood'].argmax()]
legend_labels.append(r'$\textrm{DSC + MF} + w(p) + P_\ell(k)$')
...
plt.savefig(f'fig/posterior_HOD_c{cosmo_idx:03}_hod{hod_idx:03}.pdf', bbox_inches='tight')
plt.savefig(f'fig/posterior_HOD_c{cosmo_idx:03}_hod{hod_idx:03}.png', bbox_inches='tight', dpi=300)
