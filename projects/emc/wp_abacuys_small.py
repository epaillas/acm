...
def compute_tpcf(data_positions, edges, boxsize, nthreads=4, gpu=True, los='z'):
    return TwoPointCorrelationFunction(
        'rp', edges=edges, data_positions1=data_positions, wrap=True,
...
for phase in phases:
    ...
    wp = compute_tpcf(hod_positions, edges, boxsize, nthreads=256, gpu=False, los=los)

    save_dir = '/pscratch/sd/e/epaillas/emc/covariance_sets/wp/z0.5/yuan23_prior/'
    Path(save_dir).mkdir(parents=True, exist_ok=True)
    save_fn = Path(save_dir) / f'wp_ph{phase:03}_hod{hod:03}.npy'
    wp.save(save_fn)