...
# Modified script to change the prints by a logger to speed up the process
# (prints are slow in python)
import logging
import time, sys
...
def prepare_slab(i, savedir, simdir, simname, z_mock, tracer_flags, MT, want_ranks, want_AB, cleaning, newseed,
                 halo_lc=False, nthread = 1, overwrite = 1, verbose = 1, mcut = 1e11, rad_outer = 5.):
    outfilename_halos = savedir+'/halos_xcom_'+str(i)+'_seed'+str(newseed)+'_abacushod_oldfenv'
    outfilename_particles = savedir+'/particles_xcom_'+str(i)+'_seed'+str(newseed)+'_abacushod_oldfenv'
    # Setup a logger
    logger = logging.getLogger('prepare_sim')
    ...
    verbose = int(verbose) # convert to int to avoid issues with truth statements
    ...
    # for j in tqdm(np.arange(len(halos)), position=i, leave=False, desc=f'Slab {i}', file=progress_file): # Save the bar in a file (to avoid overlappings)
    for j in np.arange(len(halos)):
        # Custom progress bar in a file (tqdm bugs in files). 
        ...
        if j % 10000 == 0 and verbose : # Modulo 10000 to avoid too many writes
            ...
            estimated_time = elapsed_time * (len(halos) / (j + 1) - 1)
...
def main(path2config, params = None, alt_simname = None, alt_z = None, newseed = 600, halo_lc = False, overwrite = 1, verbose = 1, path2log = None):
    # Setup a logger
    logger = logging.getLogger('prepare_sim')
    ...
    os.makedirs(savedir + '/slab_progress', exist_ok=True) # create the directory if it doesn't exist
    
    # Manually overwriting the number of parallel processes and slabs (avoid doing that eventually !!)
    # numslabs = 2
    # config['prepare_sim']['Nparallel_load'] = 2
    # nthread = int(np.floor(multiprocessing.cpu_count()/config['prepare_sim']['Nparallel_load']))
    
    logger.info(f"Starting to process {numslabs} slabs with {config['prepare_sim']['Nparallel_load']} parallel processes.")

    zip_args = zip(range(numslabs), repeat(savedir),
                    ...
                    repeat(nthread), repeat(overwrite), repeat(verbose))
    ...
    with get_context("spawn").Pool(config['prepare_sim']['Nparallel_load'], 
                                   initializer=setup_logger, initargs=init_setup_args, maxtasksperchild=1) as p:
        p.starmap(prepare_slab, zip_args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path2log', help='Path to the log file (stdout if not provided)', default=None)
    ...
    parser.add_argument('--verbose',
                        help='Print progress of slabs in temporary files', default = 1, type=int
                       )
