...
def test_ngal(populated_catalog):
    assert populated_catalog.ngal == 100
...
class TestApplyDownsample:
    ...
    def test_by_nbar(self, data):
        """Downsampling by nbar should reduce the number of galaxies to the target, using boxsize to compute current nbar."""
        boxsize = lambda: np.array([10., 10., 10.])
        target_nbar = 50 / 1000.0
        result = _apply_downsample(data, tracer="FOO", n_gal=None, f_gal=None, nbar=target_nbar, boxsize=boxsize)
        assert len(result) == 50

    def test_nbar_without_boxsize_raises(self, data):
        """Downsampling by nbar without providing boxsize should raise an error."""
...
def test_ngal_multi_tracer(multi_tracer_catalog):
    ...
    assert multi_tracer_catalog.ngal == 150

def test_ngal_per_tracer(multi_tracer_catalog):
    """_ngal should return the correct count per tracer independently."""
    assert multi_tracer_catalog._ngal("FOO") == 100
    assert multi_tracer_catalog._ngal("BAR") == 50
